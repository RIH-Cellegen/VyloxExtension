...
func create_float(var_name: String, editable: bool, min_value: float, max_value: float, transition_value := 0.1) -> VInspector:
	if transition_value >= 1: return inspector(var_name, Variable_type.Float, Property_usage.NONE, Property_hint.RANGE, str(min_value) + "," + str(max_value) + "," + str(-transition_value)) if editable == true else inspector(var_name, Variable_type.Float, Property_usage.READ_ONLY, Property_hint.RANGE, str(min_value) + "," + str(max_value) + "," + str(-transition_value))
	else: return inspector(var_name, Variable_type.Float, Property_usage.NONE, Property_hint.RANGE, str(min_value) + "," + str(max_value) + "," + str(transition_value)) if editable == true else inspector(var_name, Variable_type.Float, Property_usage.READ_ONLY, Property_hint.RANGE, str(min_value) + "," + str(max_value) + "," + str(transition_value))
...
func create_vector2(var_name: String, editable: bool, intigerize := false) -> VInspector:
	if intigerize: return inspector(var_name, Variable_type.Vector2i) if editable == true else inspector(var_name, Variable_type.Vector2i, Property_usage.READ_ONLY)
	else: return inspector(var_name, Variable_type.Vector2) if editable == true else inspector(var_name, Variable_type.Vector2, Property_usage.READ_ONLY)
...
func create_category(var_name: String) -> VInspector:
...
func create_enum(var_name: String, enum_var: Dictionary, editable: bool, include_values := true, capitalize_words := false) -> VInspector:
	if editable: return inspector(var_name, Variable_type.Int, Property_usage.UPDATE_ALL_IF_MODIFIED, Property_hint.ENUM, enum2str(enum_var, include_values, capitalize_words))
	else: return inspector(var_name, Variable_type.Int, Property_usage.READ_ONLY, Property_hint.ENUM, enum2str(enum_var, include_values, capitalize_words))
